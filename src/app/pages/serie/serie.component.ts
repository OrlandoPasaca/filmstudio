...
export class SerieComponent implements OnInit {
  ...
  cantbolas=Array(6);
  id:number;
  tipo_slider:string[]=["nuevos_capitulos"];
  titulo_slider:string[]=["Nuevos Capitulos"]
  posicion:number;
  urlimg:string;
  stylebola:object[];
  timeout:number;
...
async cambiarimg()
{
  ...
  const id_categoria=await this.service.obtener_id_serie(this.posicion).toPromise();
